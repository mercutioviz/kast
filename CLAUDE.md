...
- **Test collection errors.** `python -m pytest kast/tests/ --collect-only` reports 505 tests but 5 collection errors: `test_ai_anthropic_adapter`, `test_related_sites_config`, `test_related_sites_error_handling`, `test_related_sites_filtering`, `test_script_detection_config`. Investigate and fix.
- **`kast/scripts/create_plugin.py` wizard.** Predates `ExternalToolPlugin` and emits v2-shaped output. Either delete (since `template_plugin.py` is the canonical starter) or rewrite to emit v3-shaped output.
