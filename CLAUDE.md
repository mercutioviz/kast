...
- **CLI dispatch in `kast/cli/`** — LANDED (Phase B1+B7). The Click-based dispatcher is at `kast/cli/main.py` with subcommands `scan`, `plugins list/deps`, `config schema/init/show`, and `version`. `kast/cli/__init__.py` holds `_translate_v2_argv()` which maps legacy v2 invocations (`kast --target X`, `kast --list-plugins`, etc.) to the v3 subcommand form before Click parses them — this preserves the kast↔kast-web argv contract documented in `docs/web-integration.md`. **`kast/main.py` is now a 5-line shim that delegates to `kast.cli.main()`.** When adding new subcommands, declare them in `kast/cli/main.py` (or, as B2/B4/B5/B6 progress, in their own module under `kast/cli/`). Do NOT reintroduce argparse logic in `kast/main.py`. The `--zap-profile` relative-path bug (audit § 5.3) is **fixed** in the v3 scan subcommand by resolving relative to the package directory.
