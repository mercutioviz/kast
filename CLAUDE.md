...
- **PluginRegistry at `kast/registry.py`** — LANDED (Phase A3). Construct one per kast invocation: `registry = PluginRegistry(logger, cli_args=args, config_manager=cm)`. Use `registry.discover()` for class list, `registry.all_instances()` for cached instances (sorted by priority), `registry.get(name)` for a specific plugin, `registry.filter_by_mode(mode)` for instances filtered by `scan_type`. Tolerates the legacy `__init__(cli_args)` signature via internal TypeError fallback (removed in Phase A5). Phase A4 migrates the five existing call sites in `main.py`, `orchestrator.py`, and `utils.py` to use the registry — until A4 lands, both the registry and the legacy `discover_plugins()` calls coexist.
